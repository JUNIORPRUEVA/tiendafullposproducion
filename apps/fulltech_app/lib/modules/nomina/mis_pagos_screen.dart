...
class _MisPagosScreenState extends ConsumerState<MisPagosScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalHistorico = _items.fold<double>(0, (sum, item) => sum + item.netTotal);
    ...
    final proximoPago = _items
        .where((item) => item.periodStatus.toUpperCase() == 'DRAFT' || item.periodStatus.toUpperCase() == 'CLOSED')
        .fold<double>(0, (sum, item) => sum + item.netTotal);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _SummaryLine(label: 'Total pagado histórico', value: totalHistorico),
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        trailing: Chip(
          ...
          backgroundColor: item.isPaid ? Colors.green.withValues(alpha: 0.15) : Colors.orange.withValues(alpha: 0.15),
        ),
        ...
        children: [
          ...
          _Line(label: 'Extras', value: item.overtimeAmount + item.bonusesAmount),
